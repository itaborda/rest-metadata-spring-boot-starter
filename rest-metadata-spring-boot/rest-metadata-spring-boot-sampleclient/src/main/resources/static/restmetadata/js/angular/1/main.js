 ...
 * 
 */
var appModule = angular.module("RestMetadata", [ 'ui.bootstrap' ]);
		...
		.factory(
				...
				function($compile, $http) {
					...
					var UIForm = (function() {
						...
						this.buildForm = function(jsonData, rootContext, $scope, elId) {
						...
						this.make = function(jsonData, rootContext, $scope) {
							...
							for (var g_i = 0; g_i < $scope.result.groupFields.length; g_i++) {
								...
								var key = Object.keys(groupField)[0];
								groupField = groupField[key];
								var group = $scope.result.groups[key];
								...
								for (var f_i = 0; f_i < groupField.length; f_i++) {

									var field = groupField[f_i];
						...
						var Utils = {
							...
							checkEntry : function(entry) {
								...
								return entry && entry.hasOwnProperty("value") ? entry.value : entry;
							},

							checkEntryValue : function(entry, value) {
								if (entry && entry.hasOwnProperty("value"))
									entry.value = value;
								else
									entry = value;
